...
import { toBrazilTime } from "../utils/timezone.utils"
...
export async function createAppointmentService(input: {
  ...
  const requestedStartBrazil = typeof parse.data.date === "string" ? new Date(parse.data.date) : parse.data.date
  if (Number.isNaN(requestedStartBrazil.getTime())) {
    return { success: false, error: "Data inválida" }
  }

  // Step A: Fetch service duration from DB to calculate endTime
  // (service já foi buscado acima com duration)
  const startUtc = toBrazilTime(requestedStartBrazil)
  ...
  const withinWork = workSpans.some((span) => {
    ...
    const s = new Date(requestedStartBrazil)
    s.setHours(sh, sm, 0, 0)
    const startSpanUtc = toBrazilTime(s)
    
    const e = new Date(requestedStartBrazil)
    e.setHours(eh, em, 0, 0)
    const endSpanUtc = toBrazilTime(e)
