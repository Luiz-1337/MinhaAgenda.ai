...
export interface SendMarketingMessageOptions {
  /** True when the body was generated by an LLM — caller may choose to add a typing indicator. */
...
export type SendMarketingMessage = (
  ...
  body: string,
  salonId: string,
  options?: SendMarketingMessageOptions
) => Promise<void>
...
export async function dispatchPendingMessages(
  ...
  limit = 100
): Promise<{ sentCount: number; failedCount: number; skippedOptedOut: number }> {
  // Defense-in-depth: even if the enqueue stage missed an opt-out (race during
  // the scheduled window), this LEFT JOIN + filter prevents sending to anyone
  // whose `customers.opted_out_at` is set.
  const pending = await db.execute(sql`
    select
      ...
      cm.generated_by_ai as generated_by_ai,
      c.salon_id as salon_id,
      cu.opted_out_at as opted_out_at
  ...
  for (const row of pending) {
    ...
    if (row.opted_out_at) {
      await db
        ...
        .set({ status: 'skipped_opt_out', errorMessage: 'customer opted out' })
  ...
  return { sentCount, failedCount, skippedOptedOut }
}

export async function runMarketingDispatcher(
  sendMessage: SendMarketingMessage
): Promise<MarketingDispatcherResult & { skippedOptedOut: number }> {
  const queuedCount = await enqueueRecoveryMessages()
  const { sentCount, failedCount, skippedOptedOut } = await dispatchPendingMessages(sendMessage)

  return { queuedCount, sentCount, failedCount, skippedOptedOut }
}
