...
async function main() {
  const file = path.resolve(__dirname, '..', 'drizzle', '0039_professional_person_key_and_specialist.sql')
  ...
  console.log('migrate-039-ok')
  await sql.end({ timeout: 0 })
}

main().catch((err) => {
  console.error('migrate-039-error', err)
  process.exit(1)
})
