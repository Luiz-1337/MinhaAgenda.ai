...
async function main() {
  ...
  const file = resolve(__dirname, '..', '..', '..', 'supabase', 'migrations', '005_add_profile_fields.sql')
  ...
  console.log('Aplicando migração 005_add_profile_fields.sql...')
  ...
  try {
    ...
    if (err.code === '42P07' || err.code === '42710' || err.code === '42701' || err.code === '23505' || err.code === '42P16') {
      console.log(`⚠ Coluna já existe ou objeto já criado: ${err.code} - ${err.message?.split('\n')[0]}`)
