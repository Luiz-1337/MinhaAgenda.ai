...
export default function MarketingClient({ salonId }: { salonId: string }) {
  ...
  useEffect(() => {
    ...
    startLoading(async () => {
      ...
      try {
        const result = await getRecoveryFlow(salonId);
        if ("error" in result) {
  ...
  const handleSendCampaign = () => {
    ...
    startSending(async () => {
      ...
      const sendResult = await sendBroadcastCampaign(createResult.data.campaignId, salonId);
  ...
  const handleOpenLeadsModal = () => {
    ...
    startLeadsLoading(async () => {
      const result = await listSegmentedLeads(criteria, salonId);
      if ("error" in result) {
        setLeadsList([]);
