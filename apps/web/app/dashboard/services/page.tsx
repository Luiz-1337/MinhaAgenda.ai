...
import { getServices, upsertService, deleteService, getServiceLinkedProfessionals, type ServiceRow } from "@/app/actions/services"
...
export default function ServicesPage() {
  ...
  useEffect(() => {
    startTransition(async () => {
      ...
      if (Array.isArray(pros)) setTeam(pros)

      if (Array.isArray(res)) {
        const map: Record<string, string[]> = {}
        for (const s of res) {
          const linked = await getServiceLinkedProfessionals(s.id)
          if (Array.isArray(linked)) map[s.id] = linked
        }
        setLinks(map)
      }
    })
