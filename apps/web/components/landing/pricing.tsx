    ...
    <section id="plans" className="py-24 bg-background border-t border-border/50">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="mb-14"
        >
          <p className="text-xs font-semibold text-accent uppercase tracking-widest mb-3">Planos</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-5xl mx-auto items-stretch">
          {PLANS.map((plan, index) => {
            ...
            const href = isEnterprise ? '/contact' : `/register?plan=${plan.name}`
            ...
            return (
              <motion.div
                ...
                whileHover={{ scale: 1.04, transition: { type: "tween", duration: 0.15 } }}
                className={`relative flex flex-col rounded-md p-6 transition-all cursor-pointer ${
                  plan.highlight
                    ? 'bg-primary text-primary-foreground ring-1 ring-primary/20 z-10'
                ...
                <ul className="space-y-3 flex-1 mb-6">
                  {plan.features.map((feature, fIndex) => (
                    <li key={fIndex} className="flex items-start gap-2.5">
                      <Check className={`h-4 w-4 shrink-0 mt-0.5 ${plan.highlight ? 'text-primary-foreground/80' : 'text-accent'}`} />
                  ...
                  <button
                    className={`w-full py-2.5 px-5 rounded-md text-sm font-semibold transition-colors ${
                      plan.highlight
                        ...
                        : 'bg-muted text-foreground hover:bg-accent hover:text-accent-foreground border border-border hover:border-accent'
                    }`}
                  >
                    {plan.buttonText}
                  </button>
